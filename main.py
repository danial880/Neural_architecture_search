...
def main():
    ...
    search_model = ModelSearch(args, class_labels)
    # Fetch the desired sub dataset.
    # Run search on the fetched sub dataset.
    results1 = search_model.search_depth_and_width(args, class_labels,
                                                   trainer.train_test,
                                                   trainer.train)
    ...
    d_w_model_info = {'curr_arch_ops': curr_arch_ops,
                      ...
                      'f_layers': f_layers}
    results2 = search_model.search_operations_and_kernels(args, d_w_model_info,
                                                          class_labels,
                                                          trainer.train_test,
                                                          trainer.train)
    ...
    model = NetworkMixArch(f_channels, len(np.array(class_labels)), f_layers,
                           curr_arch_ops, curr_arch_kernel, args.input_shape)
    model = model.cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--report_freq', type=float, default=30,
    ...
    parser.add_argument('--save', type=str, default='hard-ds',
                        help='experiment name')
    parser.add_argument('--seed', type=int, default=0, help='random seed')
    parser.add_argument('--target_acc', type=float, default=99.00,
                        help='desired target accuracy')
    parser.add_argument('--target_acc_tolerance', type=float, default=0.5,
                        help='tolerance for desired target accuracy')
    parser.add_argument('--ch_drop_tolerance', type=float, default=0.5,
                        help='tolerance when dropping channels')
    parser.add_argument('--min_width', type=int, default=16,
                        help='minimum number of init channels in search')
    parser.add_argument('--max_width', type=int, default=64,
                        help='maximum number of init channels in search')
    parser.add_argument('--width_resolution', type=int, default=16,
                        help='resolution for number of channels search')
    parser.add_argument('--min_depth', type=int, default=5,
                        help='minimum number of init layers in search')
    parser.add_argument('--max_depth', type=int, default=15,
                        help='maximum number of init layers in search')
