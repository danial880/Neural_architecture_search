"""An experimental class to extract data subsets with correct labels no matter
   which classes
"""
import torch
import numpy as np
...
def get_data_subsets(dataset, class_labels):
    ...
    # Because the originally selected class labels maybe random, therefore
    # renaming them.
    ...
    dataset = MySubset(dataset, inds, torch.from_numpy(new_targets))
    return dataset
